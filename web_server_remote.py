...
@app.get("/api/sub/clash")
def get_clash_subscription():
    """从新加坡 VPS 的 X-UI 后台实时同步最新配置并生成 Clash 订阅"""
    ...
    try:
        # 1. 登录 X-UI 后台
        base_url = "http://217.15.163.138:54321"
        ...
        r_login = session.post(login_url, data=payload, timeout=8)
        if r_login.status_code != 200 or not r_login.json().get("success"):
            return PlainTextResponse(content="Error: Failed to log in to X-UI panel.", status_code=500)
            
        # 2. 获取 inbound 列表
        list_url = f"{base_url}/xui/inbound/list"
        r_list = session.post(list_url, timeout=8)
        if r_list.status_code != 200 or not r_list.json().get("success"):
            return PlainTextResponse(content="Error: Failed to fetch inbounds from X-UI.", status_code=500)
            
        inbounds = r_list.json().get("obj", [])
        if not inbounds:
            return PlainTextResponse(content="Error: No inbound configuration found on X-UI.", status_code=404)
            
        # 找到 remark 为 "sg" 或者是第一个入站
        inbound = inbounds[0]
        ...
        if not clients:
            return PlainTextResponse(content="Error: No clients configured on X-UI.", status_code=404)
            
        # 取 User1 
        client = clients[0]
        uuid = client.get("id")
        flow = client.get("flow", "")
        fp = client.get("fingerprint", "chrome")
        
        # 解析 streamSettings (Reality, SNI, pbk, sid)
        stream_settings = json.loads(inbound.get("streamSettings", "{}"))
        reality_settings = stream_settings.get("realitySettings", {})
        
        server_names = reality_settings.get("serverNames", ["apple.com"])
        ...
        sid_part = f"\n      short-id: {sid}" if sid else ""
        
        yaml_content = f"""# Clash Verge Rev 办公订阅配置文件 (Contabo 新加坡 Reality)
...
rules:
  ...
  - DOMAIN-KEYWORD,byte,DIRECT
  - DOMAIN-KEYWORD,zijie,DIRECT
  - DOMAIN-KEYWORD,ndcpp,DIRECT
...
"""
        ...
        return PlainTextResponse(content=f"Error syncing with X-UI: {str(e)}", status_code=500)
